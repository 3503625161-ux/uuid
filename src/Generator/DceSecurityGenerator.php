...
class DceSecurityGenerator implements DceSecurityGeneratorInterface
{
    ...
    private function resolveLocalIdentifier(
        int $localDomain,
        ?IntegerObject $localIdentifier,
    ): IntegerObject {
        if ($localIdentifier !== null && $localIdentifier->isNegative()) {
            throw new DceSecurityException(
                'Local identifier out of bounds; it must be a value between 0 and 4294967295',
            );
        }

        $resolved = match ($localDomain) {
            Uuid::DCE_DOMAIN_ORG => $localIdentifier ?? throw new DceSecurityException(
                'A local identifier must be provided for the org domain',
            ),
            Uuid::DCE_DOMAIN_PERSON => $localIdentifier ?? $this->dceSecurityProvider->getUid(),
            Uuid::DCE_DOMAIN_GROUP => $localIdentifier ?? $this->dceSecurityProvider->getGid(),
        };
